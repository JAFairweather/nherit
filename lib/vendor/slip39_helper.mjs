// slip39_helper.mjs — vendored from slip39-js v0.1.9 (MIT, (c) 2019 Pal
// Dorogi "ilap" <pal.dorogi@gmail.com>, github.com/ilap/slip39-js), converted
// to ESM with the three Node-crypto call sites replaced by audited
// @noble/hashes primitives so the SAME file runs in Node and the browser
// (esm.sh's node:crypto shim lacks pbkdf2Sync). The SLIP-39 math, wordlist,
// and structure are otherwise untouched. Vendored because upstream is
// CommonJS + node:crypto only.
...
function randomBytes(length = 32) {
  let randoms = crypto.getRandomValues(new Uint8Array(length)) /* global webcrypto: Node >=20 and browsers */;
...
function roundFunction(round, passphrase, exp, salt, secret) {
  ...
  const key = pbkdf2(sha256, Uint8Array.from(roundedPhrase), Uint8Array.from(saltedSecret), {
    c: count,
    dkLen: secret.length,
  });
...
function createDigest(randomData, sharedSecret) {
  let result = hmac(sha256, Uint8Array.from(randomData), Uint8Array.from(sharedSecret));
